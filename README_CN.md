...
- 备忘录功能：基于 NVS 持久化的备忘录（最多 8 条，每条 60 字），支持可选的定时提醒。可从菜单页或通过小智 MCP 工具（`self.memo.add`、`self.memo.list`、`self.memo.remove`）操作。到期提醒会在表情页显示并发出提示音。
...
- 备忘录提醒依赖 NTP 时间同步；如果 NTP 未同步，提醒时间将相对于设备启动时间。
