...
4. Read the UI classes under `src/ui/`, starting with `face_ui.*`, `menu_ui.*`, and `camera_debug_ui.*`.
...
7. Read `src/servo/servo_motion_controller.*` before changing servo behavior, because direct large servo jumps can cause brownout resets.
