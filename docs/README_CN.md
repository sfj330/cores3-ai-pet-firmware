...
7. 如果你想理解局域网控制和网页相册，再读 `src/network/web_server.*`。
...
- 外设利用说明优先引用根目录 `README_CN.md` 的"实验报告可引用的现有外设利用"小节。
- 当前方案不新增 System Resources UI，也不新增菜单入口；报告加分通过现有功能覆盖 LCD、触摸、IMU、摄像头、麦克风、扬声器、SD、Wi-Fi、PMU/NVS、PCA9685、ADC、RTC 和 DMA。
- 写报告时注意区分“启发式人脸区域估计”和“人脸识别”，不要把开环舵机姿态描述成 PID 闭环云台。
