...
- 相机任务现在按目标 20 FPS 做基于耗时的节拍控制，不再只是在每帧后固定延时，预览节奏会更均匀。
...
如果网页能打开但实时预览一直空白，记得 `/api/stream` 只是镜像已经运行中的前台相机会话，需要先在设备上打开 Camera Debug 或 AI Vision。

## 网页 OTA 失败或设备没有重启

检查：

- 当前构建是否使用了 `board_build.partitions = default_16MB.csv`。
- 上传的是否是 `[env:m5stack-cores3]` 编译出的正确 `.bin`。
- 上传和重启全过程供电是否稳定。
- 串口里是否出现 `No OTA partition`、`OTA begin failed`、`Write failed`、`OTA end failed`。

说明：

- 网页 OTA 直接通过 ESP-IDF OTA API 写入下一个启动分区，写完后切换启动槽并重启。
- 这个 HTTP OTA 接口没有鉴权，只适合在可信局域网里使用。
...
- 系统页或串口日志里的电池电压。
- 电压是否持续约 30 秒低于 `3.2 V`。

当前行为：

- 低于低电量阈值时，表情页可能被切到 `SLEEPY`，并减少表情页视觉任务负载。
- 如果持续低于临界阈值，固件会主动进入带定时唤醒的深度休眠，而不是一直运行到硬件 brownout。
